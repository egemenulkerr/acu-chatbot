...
async def startup_event():
    ...
    Ağır yüklenen bileşenleri (model, Zemberek, cihaz verisi) başlatıyoruz.
    """

    # 1) Intent verilerini ve embedding modelini yükle (senkron - işlem tamamlanana kadar bekle)
    print("📚 Intent verileri ve modeller yükleniyor...")
    await asyncio.to_thread(load_intent_data)

    # 2) Cihaz veritabanını yükle (senkron)
    print("🔧 Cihaz veritabanı yükleniyor...")
    await asyncio.to_thread(initialize_device_db)

    # 3) HIZLI: Sadece yemek verilerini arka planda güncelle (takvim scraper slow)
    print("🍽️ Yemek listesi arka planda güncelleniyor...")
    asyncio.create_task(asyncio.to_thread(update_system_data_fast))
