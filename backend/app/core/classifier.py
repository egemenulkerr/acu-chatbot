...
KEYWORD_THRESHOLD = 10      # Adım 1 Eşiği (Puan)
SIMILARITY_THRESHOLD = 0.6  # Adım 2 Eşiği (Yüzde)
...
def load_intent_data():
    ...
    try:
        with open("app/data/intents.json", "r", encoding="utf-8") as f:
            ...
            # Ayarları al
            KEYWORD_THRESHOLD = data.get("keyword_threshold", 10)
            SIMILARITY_THRESHOLD = data.get("similarity_threshold", 0.6)
            
            # Adım 2 için vektörleri hazırla
            ...
            logger.info("Tüm niyet verileri (Keywords + Embeddings) başarıyla yüklendi.")
...
def classify_intent(user_message: str):
    ...
    2. Adım: Anlamsal Arama (Semantic Search)
